    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={[styles.title, { color: theme.secondaryColor || '#0F172A' }]}>Team niet gevonden</Text>
        </View>
      </View>
    );
  }
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={[styles.title, { color: theme.secondaryColor || '#0F172A' }]}>Team details</Text>
      ...
      <View style={styles.section}>
        ...
        {ploegbaasInfo ? (
          <TouchableOpacity style={styles.ploegbaasCard} onPress={() => router.push(`/admin/werknemer-detail?id=${ploegbaasInfo.id}` as any)}>
            <View style={[styles.ploegbaasAvatar, { backgroundColor: theme.primaryColor || '#22C55E' }]}>
            ...
            <View style={[styles.ploegbaasBadge, { backgroundColor: `${theme.primaryColor || '#22C55E'}20` }]}>
              <Ionicons name="star" size={16} color={theme.primaryColor || '#22C55E'} />
              <Text style={[styles.ploegbaasBadgeText, { color: theme.primaryColor || '#22C55E' }]}>Ploegbaas</Text>
            </View>
          </TouchableOpacity>
        ) : (
              ...
              <TouchableOpacity key={lid.id} style={styles.lidCard} onPress={() => router.push(`/admin/werknemer-detail?id=${lid.id}` as any)}>
                ...
                {lid.naam === team.ploegbaas && (
                  <View style={styles.starBadge}>
                    <Ionicons name="star" size={14} color={theme.primaryColor || '#22C55E'} />
                  </View>
                )}
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              ...
              <ScrollView horizontal style={styles.ploegbaasSelector}>
                <TouchableOpacity style={[styles.selectOption, !formData.ploegbaas && styles.selectOptionActive, !formData.ploegbaas && { backgroundColor: theme.primaryColor || '#22C55E', borderColor: theme.primaryColor || '#22C55E' }]} onPress={() => setFormData({ ...formData, ploegbaas: '' })}>
                  <Text style={[styles.selectOptionText, !formData.ploegbaas && styles.selectOptionTextActive]}>Geen</Text>
                </TouchableOpacity>
                {formData.leden.map((naam) => (
                  <TouchableOpacity key={naam} style={[styles.selectOption, formData.ploegbaas === naam && styles.selectOptionActive, formData.ploegbaas === naam && { backgroundColor: theme.primaryColor || '#22C55E', borderColor: theme.primaryColor || '#22C55E' }]} onPress={() => setFormData({ ...formData, ploegbaas: naam })}>
...
const styles = StyleSheet.create({
  ...
  teamName: { fontSize: 28, fontWeight: '700', color: '#0F172A' },
  ...
  ploegbaasAvatar: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#22C55E', alignItems: 'center', justifyContent: 'center' },
  ploegbaasAvatarText: { fontSize: 20, fontWeight: '600', color: '#fff' },
  ploegbaasInfo: { flex: 1, marginLeft: 12 },
  ploegbaasName: { fontSize: 16, fontWeight: '600', color: '#0F172A' },
  ploegbaasEmail: { fontSize: 13, color: '#6c757d', marginTop: 2 },
  ploegbaasBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#22C55E20', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6 },
  ploegbaasBadgeText: { fontSize: 12, fontWeight: '600', color: '#22C55E' },
  ...
  lidName: { fontSize: 15, fontWeight: '500', color: '#0F172A' },
  ...
  selectOptionActive: { backgroundColor: '#22C55E', borderColor: '#22C55E' },
  ...
  werknemerNaam: { flex: 1, fontSize: 15, color: '#0F172A' },
