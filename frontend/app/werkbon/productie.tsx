/**
 * Productie Werkbon - Unified System
...
import { werkbonStyles, LEGAL_TEXT } from '../../components/werkbon';
...
export default function ProductieWerkbonScreen() {
  ...
  const validatePage1 = () => {
    ...
    if (!productType.trim()) {
      Alert.alert('Fout', 'Vul het product type in');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        hoeveelheid: parseFloat(hoeveelheid) || 0,
        eenheid,
        opmerking,
      ...
      const createRes = await axios.post(
        `${BACKEND_URL}/api/productie-werkbonnen?user_id=${user?.id}`,
        payload
      );
      ...
      if (werkbonId) {
        // Send PDF
        await axios.post(
          `${BACKEND_URL}/api/productie-werkbonnen/${werkbonId}/verzenden?user_id=${user?.id}`
        );
      ...
      >
        {page === 1 ? (
          ...
          <View style={styles.signaturePage}>
            ...
            {/* FIXED Signature Canvas - NOT inside ScrollView! */}
