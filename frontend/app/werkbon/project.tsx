...
export default function ProjectWerkbonScreen() {
  ...
  const handleSave = async (sendEmail: boolean) => {
    ...
    try {
      ...
      // Safely filter and map taken array
      const safeTaken = Array.isArray(taken) 
        ? taken.filter(t => t && typeof t === 'string' && t.trim()).map(t => t.trim())
        : [];
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    fontSize: 16, color: '#0F172A', borderWidth: 1, borderColor: '#E8E9ED', minHeight: 52,
  },
