...
export default function OpleveringWerkbonScreen() {
  ...
  const submit = async (signature: string) => {
    ...
    try {
      ...
      const sendResponse = await fetch(`${API_URL}/api/oplevering-werkbonnen/${werkbonId}/verzenden${query}`, { method: 'POST' });
      ...
      if (!sendResponse.ok) {
        ...
        showAlert(
          'Werkbon Opgeslagen',
          `De oplevering werkbon is succesvol opgeslagen, maar de e-mail kon niet worden verzonden: ${sendError.detail || 'Onbekende fout'}`
        );
        router.back();
        return;
      }
      ...
      if (sendData.email_sent) {
        showAlert('Gelukt', 'Oplevering werkbon opgeslagen en PDF verzonden naar: ' + (sendData.recipients?.join(', ') || 'ontvangers'));
      } else {
        showAlert(
          'Werkbon Opgeslagen',
      ...
      console.error('Oplevering werkbon error:', error);
